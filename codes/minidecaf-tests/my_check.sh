...
if [[ $STEP == 0 ]]; then
    echo "STEP=0: no tests run"
    exit 0
elif [[ $STEP == 1 ]]; then # fvck bash cause ya can't do `testcases/step{1}/*.c`
    JOBS=($(eval echo testcases/step1/*.c))
    FAILJOBS=($(eval echo failcases/step1/*.c))
else
    JOBS=($(eval echo testcases/step{$(s=(`seq ${STEP}`); IFS=, ; echo "${s[*]}")}/*.c))
    FAILJOBS=($(eval echo failcases/step{$(s=(`seq ${STEP}`); IFS=, ; echo "${s[*]}")}/*.c))
fi
...
check_env_and_parallel() {
    ...
    if $PARALLEL && parallel --version >/dev/null 2>&1; then
